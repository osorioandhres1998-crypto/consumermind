...
/**
 * LANDING ANALYZER — módulo del master-tool.
 * Audita una landing real contra los estándares de alta conversión
 * (motor determinista en el backend). Réplica del layout de referencia:
 * score, principios activos, alertas éticas y semáforo ético.
 */
  ...
  const run = async (e) => {
    ...
    try {
      const data = await apiFetch('/api/landing/analyze', {
        method: 'POST',
        body: JSON.stringify({ url, projectId: id, html: showPaste && pastedHtml.trim() ? pastedHtml : undefined }),
      });
      setResult(data.result);
      if (!showPaste && data.result?.fetch_meta?.js_suspected) setShowPaste(true);
    } catch (err) {
      setError(err.message);
      setShowPaste(true); // fallo de descarga → ofrecer fallback
    } finally {
      setLoading(false);
    }
  };
