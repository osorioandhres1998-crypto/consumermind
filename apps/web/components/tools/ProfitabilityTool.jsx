...
import { getProject, saveMetricsSnapshot } from '../../lib/api';
import {
...
export default function ProfitabilityTool({ projectId = null }) {
  ...
  const saveSnapshot = async () => {
    ...
    try {
      await saveMetricsSnapshot({
        projectId, period: snapPeriod, source: importSource,
        metrics: {
  ...
  // Prefill del precio y vertical desde el proyecto (solo en modo proyecto).
  const [vertical, setVertical] = useState('');
  useEffect(() => {
    if (!projectId) return; // standalone: entrada 100% manual
    (async () => {
      try {
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        {showImport && (
          <div style={{ marginTop: 12 }}>
            ...
            {/* El historial mensual vive en un proyecto: en modo standalone se oculta. */}
            {projectId ? (
              <>
              ...
              <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--line)' }}>
                💡 Para guardar el mes en el historial y ver tendencias, usa esta herramienta desde un <Link href="/projects" style={{ color: 'var(--indigo-600)', fontWeight: 600 }}>proyecto</Link>.
              </p>
            )}
          </div>
        )}
